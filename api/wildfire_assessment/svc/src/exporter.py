        ...
        if isinstance(data, ee.Image):
            ...
            def download_and_validate(
                ...
                while attempt < max_retries:
                    try:
                        with requests.get(
                            download_info, stream=True, timeout=timeout
                        ) as response:
                            if response.status_code != 200:
                                raise Exception(
                                    f"Erro ao baixar {description}: {response.text}"
                                )
                            print(
                                f"Tipo de conteúdo para {description}: {response.headers.get('content-type')}"
                            )
                            output_path = os.path.join(
                                output_dir, f"{description}.{extension}"
                            )
                            if extension in ["jpeg", "jpg"]:
                            ...
                            if "image/tiff" in response.headers.get("content-type", ""):
                                ...
                                print(
                                    f"Arquivo TIFF baixado: {tmp_file_path}, tamanho: {file_size} bytes, status: {response.status_code}"
                                )
                                if response.status_code != 200:
                                    raise RuntimeError(
                                        f"Download falhou para {description}: status {response.status_code}"
                                    )
                                if file_size == 0:
                                ...
                                if header not in [b"II*\x00", b"MM\x00*"]:
                                    raise RuntimeError(
                                        f"Arquivo baixado não é um TIFF válido. Header: {header}"
                                    )
                                try:
                                    ...
                                    raise RuntimeError(
                                        f"Falha ao abrir TIFF temporário: {e}"
                                    )
                        ...
                        print(
                            f"Erro de rede ao baixar {description}: {e}. Tentando novamente ({attempt+1}/{max_retries})..."
                        )
                        time.sleep(2**attempt)
                        attempt += 1
                    except Exception as e:
                        print(f"Erro inesperado ao baixar {description}: {e}")
                        break
                return False, None, None, None
